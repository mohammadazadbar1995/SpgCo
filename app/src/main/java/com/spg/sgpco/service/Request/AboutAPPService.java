...
public class AboutAPPService {
    ...
    private static AboutAPPService aboutAPPService;

    public static AboutAPPService getInstance() {
        if (aboutAPPService == null) {
            aboutAPPService = new AboutAPPService();
        }
        return aboutAPPService;
    }


    public void aboutApp(final Resources res, final ResponseListener<AboutResponse> responseListener) {
        new ServerTransaction().setCall(res, ApiClient.getClient().create(ReqInterface.class).aboutApp(), new ServerListener() {
            @Override
