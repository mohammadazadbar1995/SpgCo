...
public class AboutSGPService {


    private static AboutSGPService aboutService;

    public static AboutSGPService getInstance() {
        if (aboutService == null) {
            aboutService = new AboutSGPService();
        }
        return aboutService;
    }


    public void aboutSGP(final Resources res, final ResponseListener<AboutResponse> responseListener) {
        new ServerTransaction().setCall(res, ApiClient.getClient().create(ReqInterface.class).aboutSGP(), new ServerListener() {
            @Override
